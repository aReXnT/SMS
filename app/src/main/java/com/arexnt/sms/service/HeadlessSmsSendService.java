...
public class HeadlessSmsSendService extends IntentService{
    ...
    private void sendAndStoreTextMessage(ContentResolver contentResolver, String[] destinations, String message) {
        ...
        Uri smsSentUri = Uri.parse(Constant.SMS_SENT_URI);

        for (String destination : destinations) {
            smsManager.sendTextMessage(destination, null, message, null, null);

            ContentValues values = new ContentValues();
            values.put(Constant.COLUMN_ADDRESS, destination);
            values.put(Constant.COLUMN_BODY, message);
